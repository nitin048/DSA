# 🔗 Copy List with Random Pointer - Complete Guide from Scratch

## 📚 Table of Contents

1. [What is Copy List with Random Pointer?](#-what-is-copy-list-with-random-pointer)
2. [Why Copy List with Random Pointer?](#-why-copy-list-with-random-pointer)
...
### **Concept Overview:**
Deep Copying Lists containing Arbitrary Random pointers.
...
## 💡 Why Copy List with Random Pointer?
...
- **Linked List Copying:** Clone nodes and set arbitrary pointers correctly.
- Step 1: Insert copy nodes inline. Step 2: Set random links. Step 3: Split lists.
...
```cpp
// Insert copy nodes inline (curr->next = new Node(curr->val, curr->next))
// Set random links: copy->random = curr->random ? curr->random->next : nullptr
```
...
- **Inline insertion eliminates the need to allocate key-value hash maps.**
